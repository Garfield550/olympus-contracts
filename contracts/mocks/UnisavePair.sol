...
contract UnisaveV2ERC20 {
  using SafeMathUnisave for uint256;
...
contract UnisaveV2Pair is UnisaveV2ERC20 {
  ...
  function getDeposited() public view returns (uint256 _deposited0, uint256 _deposited1) {
    _deposited0 = deposited0;
    _deposited1 = deposited1;
  }

  function getDummy() public view returns (uint256 _dummy0, uint256 _dummy1) {
    _dummy0 = dummy0;
    _dummy1 = dummy1;
  }
  ...
  event Deposited0Updated(uint256 deposited);
  event Deposited1Updated(uint256 deposited);
  ...
  function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
    ...
    uint256 balance0 = b0().sub(dummy0);
    uint256 balance1 = b1().sub(dummy1);
  ...
  // this low-level function should be called from a contract which performs // important safety checks
  function dummy_mint(uint256 amount0, uint256 amount1) external onlyOwner lock {
    (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
  ...
  // this low-level function should be called from a contract which performs // important safety checks
  function dummy_burn(uint256 amount0, uint256 amount1) external onlyOwner lock {
    (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
    dummy0 -= uint112(amount0);
    dummy1 -= uint112(amount1);
  ...
  function swap(
    ...
    require(amount0In > 0 || amount1In > 0, "UnisaveV2: INSUFFICIENT_INPUT_AMOUNT");
    {
      ...
      require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(1000**2), "UnisaveV2: K");
    }
  ...
  function deposit0(uint256 a) internal {
  ...
  function deposit1(uint256 a) internal {
    require(a > 0, "deposit amount must be greater than 0");
  ...
  function depositSome0(uint256 a) external onlyOwner {
    deposit0(a);
  }

  function depositSome1(uint256 a) external onlyOwner {
    deposit1(a);
  }

  function depositAll0() external onlyOwner {
    IERC20 u = IERC20(token0);
    deposit0(u.balanceOf(address(this)));
  }

  function depositAll1() external onlyOwner {
    IERC20 u = IERC20(token1);
  ...
  function set_redepositRatio0(uint16 _redpositRatio0) external onlyOwner {
  ...
  function set_redepositRatio1(uint16 _redpositRatio1) external onlyOwner {
  ...
  function _withdraw0(uint256 s) internal {
    require(s > 0, "withdraw amount must be greater than 0");
    IERC20 u = IERC20(token0);
    uint256 delta = u.balanceOf(address(this));
    IyToken y = IyToken(yToken0);
    y.withdraw(s);
  ...
  function _withdraw1(uint256 s) internal {
    require(s > 0, "withdraw amount must be greater than 0");
    IERC20 u = IERC20(token1);
    uint256 delta = u.balanceOf(address(this));
    IyToken y = IyToken(yToken1);
  ...
  function withdraw0(uint256 s) external onlyOwner {
    _withdraw0(s);
  }

  function withdraw1(uint256 s) external onlyOwner {
    _withdraw1(s);
  }
