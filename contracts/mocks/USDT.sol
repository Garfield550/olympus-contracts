...
contract HOORC20Factory {
  event HOORC20Created(address token_address, string symbol, uint256 id);
  ...
  function createToken(
    ...
    uint8 decimal
  ) public onlySuperAdmin returns (address token_address) {
  ...
  function addTokenManager(string memory symbol, address token_address) public onlySuperAdmin {
  ...
  function deleteTokenManager(string memory symbol) public onlySuperAdmin {
  ...
  function changeTokenPauser(string memory symbol, address new_pauser) public onlySuperAdmin {
    require(all_tokens[symbol] != address(0), "token not in management");
  ...
  function changeTokenOperator(string memory symbol, address new_operator) public onlySuperAdmin {
    require(all_tokens[symbol] != address(0), "token not in management");
    HOORC20Template token = HOORC20Template(all_tokens[symbol]);
  ...
  function addTokenFactor(string memory symbol, address new_factory) public onlySuperAdmin {
    require(all_tokens[symbol] != address(0), "token not in management");
    HOORC20Template token = HOORC20Template(all_tokens[symbol]);
  ...
  function deteleTokenFactor(string memory symbol, address factory) public onlySuperAdmin {
    require(all_tokens[symbol] != address(0), "token not in management");
    HOORC20Template token = HOORC20Template(all_tokens[symbol]);
  ...
  function addSuperAdmin(address new_admin) public onlySuperAdmin {
    super_admins[new_admin] = true;
    emit SuperAdminAdded(new_admin);
  }

  function removeSuperAdmin(address admin) public onlySuperAdmin {
...
contract HOORC20 is Context, IHOORC20 {
  ...
  function transferFrom(
    ...
    _approve(
      sender,
      _msgSender(),
      _allowances[sender][_msgSender()].sub(amount, "HOORC20: transfer amount exceeds allowance")
    );
    return true;
  }
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
    _approve(
      _msgSender(),
      spender,
      _allowances[_msgSender()][spender].sub(subtractedValue, "HOORC20: decreased allowance below zero")
    );
    return true;
  }
